...
--*/
{
	...
	pVcb = (PVCB)FatDevice->DeviceExtension;//从设备对象中获取卷控制块
	...
	for(i=0; i<pVcb->Bpb.RootEntries; i++) {  //表示根目录中可以容纳的目录项数量
		pDirEntry = (PDIRENT)(pBuffer + 32 * i);//获取当前目录项的指针
		//
		// 跳过未使用的目录项和被删除的目录项
		//
		if(0x0 == pDirEntry->Name[0]|| (CHAR)0xE5 == pDirEntry->Name[0])//当 Name[0] 为 0x0 时，表示该目录项未被使用；当 Name[0] 为 0xE5 时，表示该目录项对应的文件或文件夹已被删除
			continue;
		
		FatConvertDirNameToFileName(pDirEntry->Name, FileName);//目录项中的文件名从 FAT 文件系统的特定格式转换为标准的文件名格式
...
--*/
{
	...
	for (i = 2; i < pVcb->NumberOfClusters + 2; i++) {
		if (FatGetFatEntryValue(pVcb, i)==0)
			FreeClusterCount++;
	}

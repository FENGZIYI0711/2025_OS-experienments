...
STATUS
KbdRead(
	IN PDEVICE_OBJECT DeviceObject,//指向设备对象的指针，代表键盘设备
	IN PFILE_OBJECT FileObject,//指向文件对象的指针
	OUT PVOID Buffer,//指向用户提供的缓冲区的指针
	IN ULONG Request,//用户请求读取的字节数
	OUT PULONG Result OPTIONAL//用于返回实际读取的字节数
	)
{
	BOOL IntState;
	ULONG Count = 0; //记录已经读取的字节数
	PKEYBOARD_DEVICE_EXTENSION Ext = (PKEYBOARD_DEVICE_EXTENSION)DeviceObject->DeviceExtension;
	...
	while (Count < Request) { 
		...
		Count += IopReadRingBuffer(Ext->Buffer, Buffer + Count, Request - Count);//环形缓冲区的结构体指针
		
		//检查缓冲区是否为空并重置事件
		if (IopIsRingBufferEmpty(Ext->Buffer)) {
			PsResetEvent(&Ext->BufferEvent);     //事件对象从有信号状态转变为无信号状态
		}

...
///////
	//判断，实现小文件覆盖大文件还是大文件覆盖小文件
	//小文件覆盖大文件
	if(Offset + BytesToWrite < File -> FileSize)
	{
		//计算文件簇的个数
		ULONG Num = File -> FileSize / FatBytesPerCluster(&Vcb -> Bpb);
		if(File -> FileSize / FatBytesPerCluster(&Vcb -> Bpb) != 0)
		{Num ++;}
		
		// 顺着簇链向后查找写入的起始位置所在簇的簇号
		PrevClusterNum = 0;
		CurrentClusterNum = File->FirstCluster;//第一个簇号
	
		for (i = Num; i > 0; i--)
		{
			ULONG Temp = FatGetFatEntryValue(Vcb, CurrentClusterNum);//找到下一个值
			  //说明是第一簇第一个簇号
			if (CurrentClusterNum == File->FirstCluster)
			{	
			    //设置 FAT 表项的值为 0xFF8 即第一个簇设置为文件占用的最后一个簇。
				FatSetFatEntryValue(Vcb, CurrentClusterNum, 0xFF8);
				CurrentClusterNum = Temp;	//找到下一簇号
				continue;
			}
			//表项对应的簇未被文件占用代表文件没有占用簇
			FatSetFatEntryValue(Vcb, CurrentClusterNum, 0x000);
			if (Temp == 0xFF8)
				break;
			CurrentClusterNum = Temp;//找到下一簇号
		}
	...
	if (OffsetInSector + BytesToWrite > Vcb->Bpb.BytesPerSector) {
		//计算第一个扇区要写多少个字节，扇区大小减扇区偏移量
		ULONG BytesToWriteFirstTime = Vcb->Bpb.BytesPerSector - OffsetInSector;
		//计算最后一次写入的扇区要写多少数据（要写的字节数-第一个扇区写的字节数，取余每个扇区能写多少字节数）
		ULONG BytesToWriteLastTime = (BytesToWrite - BytesToWriteFirstTime) % Vcb->Bpb.BytesPerSector;
		Status = IopReadWriteSector(Vcb->DiskDevice,
									FirstSectorOfCluster,
									OffsetInSector,
									(PCHAR)Buffer,
									BytesToWriteFirstTime,
									FALSE);
		if (!EOS_SUCCESS(Status))
			return Status;
		CurrentWriteCount += BytesToWriteFirstTime;
		//去掉开始的部分和最后剩余的部分，中间的部分一定是扇区大小的整数倍，计算出这个倍数，就是要写几次中间的扇区
		int MiddleInSectorNum= (BytesToWrite - BytesToWriteFirstTime - BytesToWriteLastTime) / Vcb->Bpb.BytesPerSector;
		int j = 0;
		for (j = 0; j<MiddleInSectorNum; ++j) {
			//找到下一个簇，如果没有就先分配
			PrevClusterNum = CurrentClusterNum;
			CurrentClusterNum = FatGetFatEntryValue(Vcb, PrevClusterNum);
			...
			FirstSectorOfCluster = Vcb->FirstDataSector + (CurrentClusterNum - 2) * Vcb->Bpb.SectorsPerCluster;
			//写入当前扇区，因为写入完整的一个扇区的数据，所以偏移量为0，写入字节数为一个扇区的大小
			//写缓冲区的指针要移动到对应的位置，即起始位置+已经写完的字节数
			Status = IopReadWriteSector(Vcb->DiskDevice,
										FirstSectorOfCluster,
										0,
										(PCHAR)(Buffer + CurrentWriteCount),
										Vcb->Bpb.BytesPerSector,
										FALSE);
			if (!EOS_SUCCESS(Status))
				return Status;
			CurrentWriteCount += Vcb->Bpb.BytesPerSector;
		}
		//如果最后还剩下一些不够一个扇区的数据，写入
		if (BytesToWriteLastTime != 0) {
			//同样先找下一个簇，如果没有就分配一个
			PrevClusterNum = CurrentClusterNum;
			CurrentClusterNum = FatGetFatEntryValue(Vcb, PrevClusterNum);
			if (0 == CurrentClusterNum || CurrentClusterNum >= 0xFF8) {

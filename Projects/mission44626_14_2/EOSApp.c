...
int main(int argc, char* argv[])
{
	...
	{
        printf("Open or create file \"%s\" error: %d\n", argv[1], GetLastError());
        goto RETURN;
	}
	...
	if (argc > 2)
	{
		...
		if (4 == argc && 0 == stricmp(argv[3], "-c"))
		{
			...
			{
				// 打开现有文件失败则进行创建
				hFileWrite = CreateFile(argv[2], GENERIC_WRITE, 0, CREATE_NEW, 0);
				if (INVALID_HANDLE_VALUE == hFileWrite)
				{
			...
			// 获取源文件与目标文件的大小
			ULONG srcSize = GetFileSize(hFileRead);
			ULONG dstSize = GetFileSize(hFileWrite);
			
			// 短文件写入长文件则对目标文件进行截断
			if (srcSize < dstSize) {	
				hFileWrite = CreateFile(argv[2], GENERIC_WRITE, 0, TRUNCATE_EXISTING, 0);
			}
		}
		else
		{

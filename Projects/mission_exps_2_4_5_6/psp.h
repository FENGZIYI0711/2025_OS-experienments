...
PspCreateProcessEnvironment(
	IN UCHAR Priority,
	IN PCSTR ImageName,//创建的进程所对应的可执行文件的名称
	IN PCSTR CmdLine,//传递给新进程的命令行参数
...
PspCreateThread(
	IN PPROCESS Process,
	IN SIZE_T StackSize,//为新线程分配的栈空间大小
	IN PTHREAD_START_ROUTINE StartAddr,//指向新线程开始执行的起始函数
	IN PVOID ThreadParam,//传递给线程起始函数的参数
	IN ULONG CreateFlags,//用于指定线程的创建选项，例如线程是否立即启动、线程的优先级等
	OUT PTHREAD *Thread
	);

	...
	)
{
	...
	if (OPEN_EXISTING == CreationDisposition) {
		...
		return FatCreateNewFile(DeviceObject, FileName, FileObject);
		...
		return FatTruncateFile(DeviceObject, FileName, FileObject);
	}
}
	...
	)
{
	...
	while (BytesToWrite > BytesBeWritten) {

		if (OffsetInSector + BytesToWrite - BytesBeWritten <= Vcb->Bpb.BytesPerSector) {
			// 未超过则正常写入
			Status = IopReadWriteSector( Vcb->DiskDevice,
											FirstSectorOfCluster,
											OffsetInSector,
											((PCHAR)Buffer) + BytesBeWritten,
											BytesToWrite - BytesBeWritten,
											FALSE );
		
			if (!EOS_SUCCESS(Status))
				return Status;
				
			BytesBeWritten += BytesToWrite - BytesBeWritten;	
		}
		else {
			// 否则先全部写入当前扇区
			Status = IopReadWriteSector( Vcb->DiskDevice,
			...
			// 更新写入的字节数，便于Buffer偏移定位写入位置
			BytesBeWritten += Vcb->Bpb.BytesPerSector - OffsetInSector;
			// 下一个扇区的字节偏移量一定是0
			OffsetInSector = 0;
...
)
{
    ...
    // 检查文件是否存在。
    //
    Status = FatOpenFile((PVCB)DeviceObject->DeviceExtension, FileName, &Fcb);
    if (EOS_SUCCESS(Status)) {
        FatCloseFile(Fcb);
        return STATUS_OBJECT_NAME_COLLISION; // 文件已存在，创建失败
    }
    else if (STATUS_FILE_NOT_FOUND != Status) {
        return Status; // 其他错误
    }
    ...
    // 初始化 FCB 信息。
    //
    strncpy(Fcb->Name, FileName, sizeof(Fcb->Name) - 1);
    Fcb->Name[sizeof(Fcb->Name) - 1] = '\0';
    Fcb->AttrReadOnly = FALSE;
    Fcb->AttrHidden = FALSE;
    Fcb->AttrSystem = FALSE;
    Fcb->AttrDirectory = FALSE;
    Fcb->SharedRead = TRUE;
    Fcb->SharedWrite = TRUE;
    Fcb->FirstCluster = 0;
    Fcb->FileSize = 0;
    Fcb->OpenCount = 1;
    Fcb->ParentDirectory = NULL;
    ListInitializeHead(&Fcb->FileListEntry);
    ListInitializeHead(&Fcb->FileListHead);

    //
    // 在目录中插入目录项。
    //
    Status = FatInsertDirectoryEntry((PVCB)DeviceObject->DeviceExtension, FileName, Fcb);
    if (!EOS_SUCCESS(Status)) {
        FatFreeFcb(Fcb);
        return Status;
    }
...
)
{
    ...
    // 分配一块内存，用来读取目录扇区。
    //
    DirectorySector = (PUCHAR)MmAllocateSystemPool(SectorSize);
    if (NULL == DirectorySector) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    // 从磁盘读取根目录扇区。
    //
    Status = IopReadWriteSector(
        Vcb->DiskDevice,
        Vcb->FirstRootDirSector,
        0,
        DirectorySector,
        SectorSize,
        TRUE // TRUE表示读操作
    ...
    if (!EOS_SUCCESS(Status)) {
        MmFreeSystemPool(DirectorySector);
        return Status;
    }
    ...
    // 遍历目录扇区，寻找空闲目录项。
    //
    for (Offset = 0; Offset < SectorSize; Offset += sizeof(DIRENT)) {

        DirEntry = (PDIRENT)(DirectorySector + Offset);

        if (DirEntry->Name[0] == (CHAR)0x00 || DirEntry->Name[0] == (CHAR)0xE5) {

            //
            // 找到了空闲目录项，填充它。
            //
            FatConvertFileNameToDirName(FileName, DirEntry->Name);
            DirEntry->Attributes = DIRENT_ATTR_ARCHIVE;
            memset(DirEntry->Reserved, 0, sizeof(DirEntry->Reserved));
            DirEntry->FirstCluster = 0; // 新建时还没有数据
            DirEntry->FileSize = 0;
            ...
            MmFreeSystemPool(DirectorySector);
			Fcb->DirEntryOffset = Offset; // 更新FCB的文件偏移
            return Status;
        }
    }
...
FatAllocateFcb()
{
    ...
    Fcb = (PFCB)MmAllocateSystemPool(sizeof(FCB));
    if (Fcb != NULL) {
...
)
{
    ...
    // 打开文件。
    //
    Status = FatOpenFile(Vcb, FileName, &Fcb);
    if (!EOS_SUCCESS(Status)) {
        if (Status == STATUS_FILE_NOT_FOUND &&
            (FileObject->FlagsAndAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            return STATUS_PATH_NOT_FOUND;
        }
        return Status;
    }
    ...
    // 释放原有的所有簇。
    //
    Cluster = Fcb->FirstCluster;
    while (Cluster >= 2 && Cluster < 0xFF8) {
        USHORT Next = FatGetFatEntryValue(Vcb, Cluster);
    ...
    // 写入更新后的目录项
    //
    Status = FatWriteDirEntry(Vcb, Fcb);

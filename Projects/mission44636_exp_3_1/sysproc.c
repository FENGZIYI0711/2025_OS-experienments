...
--*/
{
	...
	// 然后设置该线程的优先级为8，恢复线程的初始时间片大小和剩余时间片大小
	 	...
	 	pThreadCtrlBlock->State = Zero;								
	 	pThreadCtrlBlock->Priority = 8;								
	 	pThreadCtrlBlock->RemainderTicks = TICKS_OF_TIME_SLICE;		
	 	pThreadCtrlBlock->InitTicks = TICKS_OF_TIME_SLICE;			
...
--*/
{
	...
	PspCurrentThread->LifeTimeTicks = TICKS_OF_TIME_SLICE * 100;  // 设置生命周期
